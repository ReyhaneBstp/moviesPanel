    ...
    <>
      <div
        className={`flex flex-col sm:flex-row sm:items-center gap-3 card p-3 sm:p-2 hover:shadow-md transition-all duration-200 ${
          isSelected ? 'bg-secondary-800/10 ring-1 ring-secondary-500' : 'bg-white hover:bg-gray-50'
        }`}
      >
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <h4 className="hidden sm:block text-sm font-bold text-gray-800 truncate">{movie.title}</h4>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mt-2 sm:mt-1 text-xs text-gray-500">
            <span className="flex items-center gap-1"><HiOutlineCalendar size={14} className="sm:w-3 sm:h-3" />{movie.year}</span>
            <span className="flex items-center gap-1"><HiOutlineGlobeAlt size={14} className="sm:w-3 sm:h-3" />{movie.country}</span>
